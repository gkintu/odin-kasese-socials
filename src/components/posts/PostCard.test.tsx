...
const dummyPost: Post = {
  ...
  contentText:
    'Just enjoyed a wonderful hike in the Kasese hills! #Nature #Adventure',
...
describe('PostCard', () => {
  ...
  it('should render author info, content text, image, timestamp, and engagement stats', () => {
    ...
    expect(
      screen.getByAltText(`${dummyPost.author.name}'s avatar`)
    ).toBeInTheDocument();
    // Check author name
    ...
    expect(
      screen.getByText(
        formatDistanceToNow(dummyPost.timestamp, { addSuffix: true })
      )
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole('button', { name: /comment on post/i })
    ).toBeInTheDocument();
  });
  ...
  it('should render correctly without a content image', () => {
    ...
    expect(
      screen.getByText(dummyPostWithoutImage.author.name)
    ).toBeInTheDocument();
    expect(
      screen.getByText(dummyPostWithoutImage.contentText)
    ).toBeInTheDocument();
  });
});
