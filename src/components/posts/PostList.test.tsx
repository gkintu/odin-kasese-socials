...
jest.mock('./PostCard', () => {
  ...
  return ({ post }: { post: Post }) => (
    ...
    <div data-testid="mock-postcard">
      Post by {post.author.name} - ID: {post.id}
    </div>
  );
});
...
const dummyPosts: Post[] = [
  {
    id: '1',
    author: { id: 'u1', name: 'Alice', avatarUrl: '' },
    contentText: 'Post 1',
    timestamp: new Date(),
    likes: 0,
    commentsCount: 0,
  ...
  {
    id: '2',
    author: { id: 'u2', name: 'Bob', avatarUrl: '' },
    contentText: 'Post 2',
    timestamp: new Date(),
    likes: 0,
    commentsCount: 0,
...
describe('PostList', () => {
  ...
  it('should render an empty state message if no posts are provided', () => {
    ...
    expect(
      screen.getByText(/No posts to display right now./i)
    ).toBeInTheDocument();
