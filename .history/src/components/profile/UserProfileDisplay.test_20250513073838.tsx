...
describe('UserProfileDisplay', () => {
  ...
  it('falls back to the default avatar if avatarUrl prop is an empty string', () => {
    ...
    // This presumes that UserProfileDisplay.tsx implements this fallback logic.
    render(
      <UserProfileDisplay
    ...
    const avatarImage = screen.getByAltText('Profile picture');
    // The expectation is that the component will use defaultProps.avatarUrl from the spread defaultProps
    // if its internal logic handles an empty avatarUrl by using a default.
    expect(avatarImage.getAttribute('src')).toContain(encodeURIComponent(defaultProps.avatarUrl));
  });
});
