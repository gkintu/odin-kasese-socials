...
describe('UserProfileDisplay', () => {
  ...
  it('renders with default props when nothing is passed', () => {
    ...
    expect(screen.getByText('User Profile')).toBeInTheDocument(); // Default title
    expect(screen.getByText('User')).toBeInTheDocument(); // Default displayName
    expect(screen.getByText('This is a default bio.')).toBeInTheDocument(); // Default bio
  ...
  it('falls back to default avatar if avatarUrl is invalid or missing', () => {
    render(<UserProfileDisplay avatarUrl={null} />);
    const avatarImage = screen.getByAltText('User avatar');
    expect(avatarImage).toHaveAttribute('src', '/img/default-avatar.png');

    render(<UserProfileDisplay avatarUrl="" />);
    const avatarImage2 = screen.getByAltText('User avatar'); // Re-query after re-render
    expect(avatarImage2).toHaveAttribute('src', '/img/default-avatar.png');
  });
});