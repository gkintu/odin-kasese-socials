...
interface EditProfileFormProps {
  ...
  initialData: { // Initial data to pre-fill the form
    displayName: string;
    bio: string;
  };
...
const EditProfileForm: React.FC<EditProfileFormProps> = ({ onSubmit, initialData }) => {
  ...
  const {
    register,
    handleSubmit,
    control, // For controlled components if needed, or for specific field types
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          className={`mt-1 block w-full px-3 py-2 border ${errors.bio ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm`}
