...
describe('Navbar', () => {
  ...
  it('should render displayName if available, falling back to userEmail', async () => {
    ...
    // We also need to ensure the menu is open before checking for text that might be *in* the menu items if the button text isn't unique enough.
    expect(await screen.findByText(/TestUser/i)).toBeInTheDocument(); // This checks the button text
    // Check for an item within the menu to confirm it's open, e.g., the Logout button
    expect(await within(menu).findByRole('button', {name: /logout/i})).toBeInTheDocument();
  ...
  it('should handle async expectations correctly', async () => {
    ...
    await waitFor(async () => {
      ...
      // This text is on the menu button itself
