...
describe('UserListItem', () => {
  ...
  it('should render user avatar, display name', () => {
    render(<UserListItem user={dummyUserFollowing} onToggleFollow={mockToggleFollow} />);

    // Check avatar (by alt text)
    expect(screen.getByAltText(`${dummyUserFollowing.displayName}'s avatar`)).toBeInTheDocument();
    // Check display name
    expect(screen.getByText(dummyUserFollowing.displayName)).toBeInTheDocument();
  });
  ...
  it('should render "Follow" button if user.isFollowing is false', () => {
    render(<UserListItem user={dummyUserNotFollowing} onToggleFollow={mockToggleFollow} />);
