...
describe('RegistrationForm', () => {
  ...
  it('should show an error message for an invalid email format', async () => {
    ...
    await user.type(passwordInput, 'password123'); // Fill password to make it valid
    await user.click(submitButton); // Attempt to submit

    // Explicitly wait for the aria-invalid attribute to be true
    await waitFor(() => {
      expect(emailInput).toHaveAttribute('aria-invalid', 'true');
    });

    // If the above passes, then the error message should also be present
    // Using getByText now, as waitFor should ensure it's there. findByText would also work.
