...
jest.mock('@/components/posts/CreatePostForm', () => {
  const CreatePostFormMock = ({
    ...
    <form
      data-testid="mock-createpostform"
      onSubmit={() => onSubmit({ contentText: 'test content' })}
    >
      <textarea defaultValue="test content"></textarea>
...
// Define ProtectedRouteMock as a function declaration for proper hoisting
function ProtectedRouteMockComponent({ children }: { children: React.ReactNode }) {
  return <>{children}</>;
}
ProtectedRouteMockComponent.displayName = 'ProtectedRouteMock';

// Mock ProtectedRoute using the function declaration
jest.mock('@/components/Auth/ProtectedRoute', () => ProtectedRouteMockComponent);
...
describe('CreatePostPage', () => {
  ...
  it('renders CreatePostForm for authenticated non-guest user', () => {
    render(<CreatePostPage />);
    expect(screen.getByTestId('mock-createpostform')).toBeInTheDocument();
    expect(
