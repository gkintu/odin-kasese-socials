...
describe('EditProfilePage', () => {
  ...
  it('handleEditProfileSubmit updates display name and redirects', async () => {
    ...
    // mockUpdateDisplayName from beforeEach will be used unless a new one is defined here.
    // For clarity and isolation, let's define a new mock function for updateDisplayName for this specific test.
    const specificTestMockUpdateDisplayName = jest.fn();

    jest.resetModules(); // Reset modules to re-evaluate mocks

    // Re-mock useAuthStore AFTER jest.resetModules() and BEFORE requiring the page component
    // This ensures the re-imported page gets this specific mock.
    mockUseAuthStore.mockReturnValue({
      isAuthenticated: true,
      isGuest: false,
      isLoading: false,
      userEmail: 'test@example.com',
      displayName: 'Old Name Store', // Initial store display name for this test
      updateDisplayName: specificTestMockUpdateDisplayName, // Use the specific mock for this test
      // Provide other functions/properties from the store as defined in beforeEach to ensure component stability
    ...
    // Re-mock EditProfileForm AFTER jest.resetModules()
    ...
    // Re-import the page component AFTER mocks are set up
    const EditProfilePageWithInteractiveForm = require('./page').default;

    render(<EditProfilePageWithInteractiveForm />);

    const displayNameInput = screen.getByLabelText('Display Name');
    // Verify the form is initialized with the displayName from our re-mocked store
    expect(displayNameInput).toHaveValue('Old Name Store');

    await user.clear(displayNameInput);
    await user.type(displayNameInput, 'New Submitted Name'); // User types a new name

    await user.click(screen.getByRole('button', { name: /mock save interactive/i }));

    await waitFor(() => {
      // Assert that the specific mock function for this test was called
      expect(specificTestMockUpdateDisplayName).toHaveBeenCalledWith('New Submitted Name');
    });
    await waitFor(() => {
