...
jest.mock('@/components/profile/EditProfileForm', () => {
  ...
  return jest.fn(({ onSubmit }: { onSubmit: (data: { displayName: string }) => void }) => (
    <form
...
describe('EditProfilePage', () => {
  ...
  it('redirects and shows error if not authenticated', async () => {
    mockUseAuthStore.mockReturnValueOnce({
      ...
      isGuest: false,
      isLoading: false,
    } as any); // Kept as any for now, will address if it causes issues
    render(<EditProfilePage />);
    await waitFor(() =>
  ...
  it('handleEditProfileSubmit updates display name and redirects', async () => {
    ...
    // const mockSaveButton = screen.getByRole('button', {name: /mock save/i}); // unused variable
