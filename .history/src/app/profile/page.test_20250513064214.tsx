...
jest.mock('@/components/profile/UserProfileDisplay', () => {
  ...
  return ({ displayName }: { displayName?: string }) => <div data-testid="mock-user-profile-display">UserProfile: {displayName}</div>;
});
...
jest.mock('@/components/Auth/ProtectedRoute', () => {
  // eslint-disable-next-line react/display-name
  return ({ children }: { children: React.ReactNode }) => <div data-testid="mock-protected-route">{children}</div>;
});
...
describe('ProfilePage', () => {
  ...
  const setMockAuthState = (state: Partial<ReturnType<typeof useAuthStore>>) => {
    mockUseAuthStore.mockReturnValue({
      isAuthenticated: false, userEmail: null, isGuest: false, isLoading: false,
      logout: jest.fn(), error: null, loginRequest: jest.fn(), loginSuccess: jest.fn(),
      loginFailure: jest.fn(), loginAsGuest: jest.fn(),
      ...state,
    });
  };
  ...
  beforeEach(() => {
    ...
    setMockAuthState({ isLoading: false, isAuthenticated: false, isGuest: false });
  });
  ...
  it('should render guest message when user is a guest', () => {
    ...
    expect(screen.getByText((content, element) => {
      const hasText = (node: HTMLElement) => node.textContent === 'Please Login or Sign Up';
      const nodeHasText = element ? hasText(element as HTMLElement) : false;
      const childrenDontHaveText = Array.from(element?.children || []).every(
        (child) => !hasText(child as HTMLElement)
      );
      return nodeHasText && childrenDontHaveText;
    })).toBeInTheDocument();
    expect(screen.queryByTestId('mock-user-profile-display')).not.toBeInTheDocument();
  });

  it('should render UserProfileDisplay when user is authenticated', () => {
    const testEmail = 'user@example.com';
    setMockAuthState({ isAuthenticated: true, userEmail: testEmail, isLoading: false });
    render(<ProfilePage />);
    ...
    expect(profileDisplay).toHaveTextContent(`UserProfile: ${testEmail.split('@')[0]}`);
  });
  ...
  it('should render login prompt if not authenticated, not guest, and not loading (fallback within page)', () => {
    ...
    setMockAuthState({ isAuthenticated: false, isGuest: false, isLoading: false });
    render(<ProfilePage />);
