...
interface AuthState {
  ...
  loginSuccess: (email: string, initialDisplayName?: string) => void; // Modified loginSuccess
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  loginSuccess: (email, initialDisplayName) =>
    set({
      isAuthenticated: true,
      userEmail: email,
      displayName: initialDisplayName || email.split('@')[0] || get().displayName, // Use provided, derive, or keep existing as fallback logic
      isLoading: false,
      error: null,
      isGuest: false,
    }),
